...
*/
  ...
  var widgetsSortable = $.widget('ui.sortable', $.ui.mouse, {
    ...
    _mouseStart: function (event, overrideHandle, noActivation) {
      ...
      /**
       * 2017.09.21 msko
       * DP 값에 따른 event.pageX 값 차이로 drag & drop 버그 수정
       */
      var standardDP = 360; // 표준 DP
      var deviceInfo = bizMOB.Device.Info;
      var screenWidth = deviceInfo.screen_width; // 해당 디바이스 가로크기
      var targetDPI = deviceInfo.screen_density_dpi; // 해당 디바이스 집적도
      var targetDP = screenWidth / (targetDPI / 160); // 해당 디바이스 DP

      var rate = (targetDP - standardDP) / targetDP; // 표준DP와의 비율값
      var eventPageX = event.pageX;
      if (rate != 0) eventPageX = eventPageX * (1 - rate);

      $.extend(this.offset, {
        click: {
          //Where the click happened, relative to the element
          left: eventPageX - this.offset.left,
          top: event.pageY - this.offset.top,
        },
        parent: this._getParentOffset(),
